...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const id = uuidv4()
        await db.createUser(id, username, password)
...
app.get('/api/posts', auth, async (req, res) => {
    const posts = await db.getPosts(req.session.userId)
    res.json(posts)
})
...
app.post('/api/posts', auth, async (req, res) => {
    try {
        ...
        const id = uuidv4()
        await db.createPost(id, req.session.userId, imageUrl, caption, date, time, groupId)
...
app.put('/api/posts/:id', auth, async (req, res) => {
    try {
        const { imageUrl, caption, date, time, groupId } = req.body
        await db.updatePost(req.params.id, imageUrl, caption, date, time, groupId)
        res.json({ success: true })
    } catch (err) {
        res.json({ success: false, message: err.message })
    }
...
app.post('/webhook/schedule', webhookAuth, async (req, res) => {
    ...
    try {
        const id = uuidv4()
        await db.createPost(id, userId, imageUrl, caption, date, time, groupId)
